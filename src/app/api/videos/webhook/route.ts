import { Request } from "./../../../../../node_modules/@mux/mux-node/_shims/index.d";
	...
	switch (payload.type as WebhookEvent["type"]) {
		...
		case "video.asset.ready": {
			...
			if (!uploadedThumbnail.data || !uploadedPreview.data) {
				return new Response(
					"Failed to upload new thumbnail or preview",
					{ status: 500 },
				);
			}
		...
		case "video.asset.track.ready": {
			const data =
				payload.data as VideoAssetTrackReadyWebhookEvent["data"] & {
					asset_id: string; // Fix for unrecognized asset_id bug
				};
			...
			// TypeScript thinks that asset_id does not exist
			if (!assetId)
